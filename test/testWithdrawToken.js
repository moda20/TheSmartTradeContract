...
const amount  = web3.utils.toBN(web3.utils.toWei('0.01'))
...
const myAddress = "0x9F8cB701AA447f4fB5D2C5199F5c67C26C125506";
module.exports = async done => {
    try {
        //let newAccount = web3.eth.accounts.create();
        const smartTradeInstance = await smartTrade.deployed();
        const router = await routerContract.at(PancakeRouterV2);
        const amountsOutB = await router.getAmountsOut(bnbAmount, [WBNBAddress, pairC]);
        let finalamountsOutB = amountsOutB[1].mul(web3.utils.toBN(75))
            .div(web3.utils.toBN(100));
        let results = await smartTradeInstance.withdrawToken(
            myAddress,
            amount.mul(web3.utils.toBN(180)),
            WBNBAddress
        );
        console.log(results)
